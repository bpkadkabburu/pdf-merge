...
function readDir(dir) {
    return new Promise((resolve, reject) => {
        fs.readdir(dir, (err, listFile) => {
            if(err) return reject(err)

            let listBuffer = []
            listFile.forEach(file => {
                let buffer = fs.readFileSync(`${dir}\\${file}`)
                listBuffer.push(buffer)
            })

            return resolve(listBuffer)

        })
    })
}

async function mergeFile(skpdList, dir){
    for (const [index, skpd] of skpdList.entries()) {
        let skpdName = `${skpd}`
        let skpdDir = `${dir}\\6. DPA Rincian Belanja\\${skpdName}`
        let pdfBuffer = [];
        console.info(`Merging file ${skpdName}`)

        pdfBuffer = await readDir(skpdDir)
        
        console.log(`${skpdName} menggabungkan ${pdfBuffer.length} file`)
        ...
        const finalPath = "8. DPA Lengkap"

        let savePath = `${dir}\\${finalPath}\\${skpdName}.pdf`
        ...
        let deletePath = `${dir}\\${rincianBelanjaPath}\\${skpdName}\\ATAS[Merge1sampai5].pdf`
